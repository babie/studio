...
describe("integration: agent-runner turn loop", () => {
  ...
  it("aborts mid-loop when signal fires; outcome reports backend turn-not-completed", async () => {
    ...
    const r = await runAgent({
      ...
      backend,
      tracker,
      agentConfig: { backend: { type: "mock" }, maxConcurrentAgents: 1, maxTurns: 5, maxRetryBackoffMs: 300_000, agentSessionStallTimeoutMs: 1_800_000, maxConcurrentAgentsByState: {} },
      workspaceConfig: { root },
      terminalStates: [v.parse(IssueStateName.schema, "Done")],
      promptTemplate: "t",
      logger: silent,
