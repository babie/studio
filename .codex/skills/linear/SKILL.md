...
name: linear
description: |
  Use the perform `linear_graphql` client tool for raw Linear GraphQL
  operations such as comment editing and upload flows.
---

# Linear GraphQL

Use this skill for raw Linear GraphQL work during perform-driven backend
sessions.

> **Status (M3 complete, 2026-05):** the `linear_graphql` dynamic tool is a
> design carry-over from the original Symphony orchestrator. The codex backend
> can still call it through the JSON-RPC dynamic-tool path
> (`item/tool/call`, see `docs/protocol.md` §5.4). The claude-app-server
> backend does **not** implement MCP dynamic tools yet, so under
> `agent.type: claude` this skill is effectively dormant — perform itself
> drives Linear state transitions via the orchestrator
> ([ADR-0014](../../docs/adr/0014-symphony-owned-state-transitions.md)).
> Re-enable this skill once `apps/claude-app-server` ships MCP dynamic-tool
> support (TODO.md M4+).

## Primary tool

Use the `linear_graphql` client tool exposed by the perform JSON-RPC session.
It reuses perform's configured Linear auth (`linear.api_key`) for the
session.
